...
fun MySpatialContent(
    ...
    // Launch effect to initialize the ARCore session when we enter spatial mode
    LaunchedEffect(Unit) {
        arCoreManager.initSession()
    }
    
    // Render 3D anchor visualizations in the scene
    AnchorVisualizationManager(arCoreManager = arCoreManager)
    
    // UI panel for anchor management
